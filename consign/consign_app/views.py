...
def addTripSheet(request):
    ...
    if request.method == 'POST':
        ...
        # Filter consignments based only on route_to
        consignments = AddConsignmentTemp.objects.filter(route_to=route_to)
    ...
    if request.method == 'POST':
        ...
        for i in range(1, total_rows + 1):
            if str(i) in selected_rows:  # Only process if the row is selected
                ...
                pieces = int(request.POST.get(f'pieces_{i}', 0))  # Convert to int
                ...
                total_cost = float(request.POST.get(f'total_cost{i}', 0))  # Convert to float
                weightAmt = float(request.POST.get(f'weightAmt{i}', 0))  # Convert to float
                freight = float(request.POST.get(f'freight{i}', 0))  # Convert to float
                hamali = float(request.POST.get(f'hamali{i}', 0))  # Convert to float
                door_charge = float(request.POST.get(f'door_charge{i}', 0))  # Convert to float
                st_charge = float(request.POST.get(f'st_charge{i}', 0))  # Convert to float

                print(
                    f"Track ID: {track_id}, Pieces: {pieces}, Description: {desc_product}, "
                    f"Route: {route_to}, Receiver: {receiver_name}, Pay Status: {pay_status}, "
                    f"total_cost: {total_cost}, weightAmt: {weightAmt}, freight: {freight}, "
                    f"hamali: {hamali}, door_charge: {door_charge}, st_charge: {st_charge}")  # Debugging statement
...
def addTripSheetList(request):
    ...
    try:
        if request.method == 'POST':
            ...
            if date:
                # Query TripSheetTemp table based on the selected date
        ...
        if uid:
            try:
                ...
                for i in range(1, total_rows + 1):
                    ...
                    branchemail = request.POST.get(f'branchemail_{i}')
...
from django.core.exceptions import ObjectDoesNotExist
from .models import AddConsignment, Branch, DailyConsignment  # Adjust the import paths to your project structure
...
def fullLoad(request):
    if request.method == "POST":
        ...
        charge_weight = request.POST.get('charge_weight')
        ...
        Fullload.objects.create(
            ...
            consignee_mobile=consignee_mobile# Save the status (Paid or Pending)
        )

        # Redirect to the fullLoadInvoice view with the created loadId
        messages.success(request, 'Full Load created successfully.')
        return redirect('fullLoadprint', pk=con_id)  # Pass the loadId as pk
        ...
        for consignment in consignments:
            ...
            if loadId not in grouped_userdata:
                grouped_userdata[loadId] = {
                    ...
                    'loadId': consignment.loadId,
                    'consignor_name': consignment.consignor_name,
                    'consignor_gst': consignment.consignor_gst,
                    'consignor_address': consignment.consignor_address,
                    'consignee_gst': consignment.consignee_gst,
...
def vehiclePayment(request):
    vehicles = Fullload.objects.values('vehicleNo').distinct()
    return render(request,'vehiclePayment.html',{'vehicles':vehicles})

# View to fetch vehicle data
def get_vehicle_data(request):
    date = request.GET.get('date')
    vehicle_no = request.GET.get('vehicleNo')

    if not date or not vehicle_no:
        return JsonResponse({'error': 'Invalid parameters'}, status=400)

    try:
        full_load = Fullload.objects.get(date=date, vehicleNo=vehicle_no)
        data = {
            'loadId': getattr(full_load, 'loadId', ''),
            'route_from': getattr(full_load, 'route_from', ''),
            'route_to': getattr(full_load, 'route_to', ''),
            'advance': getattr(full_load, 'advance', 0),
            'total': getattr(full_load, 'total', 0),
            'finalBalance': getattr(full_load, 'finalBalance', 0),  # Ensure 0 is returned if the value is None
            'paidAmt': getattr(full_load, 'paidAmt', 0),  # Ensure 0 is returned if the value is None
        }
        return JsonResponse(data)
    except Fullload.DoesNotExist:
...
@csrf_exempt
@require_POST
def save_vehicle_payment(request):
    try:
        date = request.POST.get('date')
        ...
        final_balance = float(request.POST.get('finalBalance', 0))
        paidAmt = float(request.POST.get('paidAmt', 0))
        ltr = int(request.POST.get('ltr', 0))
        ltr_rate = float(request.POST.get('ltrRate', 0))
        rate = float(request.POST.get('rate', 0))
        def_amt = float(request.POST.get('defAmt', 0))
        toll_amt = float(request.POST.get('tollAmt', 0))
        driver_amt = float(request.POST.get('driverAmt', 0))
        tyre_amt = float(request.POST.get('tyreAmt', 0))
        driver_advance = float(request.POST.get('driverAdvance', 0))
        amt_date = request.POST.get('amtDate')

        # Create and save a new Vehiclepayment record
        Vehiclepayment.objects.create(
            date=date,
        ...
        return JsonResponse({'message': 'Vehicle payment data saved successfully'})
...
def vehiclePayReport(request):
    vehicles = Fullload.objects.values('vehicleNo').distinct()
    return render(request,'vehiclePayReport.html',{'vehicles':vehicles})


from django.db.models import Sum


def get_vehicle_payments(request):
    try:
        ...
        if not date or not vehicle_no:
            # If parameters are missing, raise an error
            raise ValueError("Missing required parameters: 'date' and 'vehicleNo'.")

        # Fetch all matching records
        payments = Vehiclepayment.objects.filter(date=date, vehicleNo=vehicle_no)

        if not payments.exists():
            # If no matching data is found, return an appropriate message
            return JsonResponse({
                'error': 'No data found for the provided date and vehicle number.',
                'date': date,
                'vehicleNo': vehicle_no
            }, json_dumps_params={'indent': 4}, status=404)

        # Calculate totals
        total_rate = payments.aggregate(Sum('rate'))['rate__sum'] or 0
        total_defAmt = payments.aggregate(Sum('defAmt'))['defAmt__sum'] or 0
        total_tollAmt = payments.aggregate(Sum('tollAmt'))['tollAmt__sum'] or 0
        total_driverAmt = payments.aggregate(Sum('driverAmt'))['driverAmt__sum'] or 0
        total_tyreAmt = payments.aggregate(Sum('tyreAmt'))['tyreAmt__sum'] or 0
        total_driverAdvance = payments.aggregate(Sum('driverAdvance'))['driverAdvance__sum'] or 0

        # Grand total of amounts
        grand_total = total_defAmt + total_tollAmt + total_driverAmt + total_tyreAmt + total_rate
        grand_total1 = total_defAmt + total_tollAmt  + total_tyreAmt + total_rate

        # Calculated value (grand_total - total_driverAdvance)
        calculated_value = total_driverAdvance - grand_total1

        # Serialize data
        payment_data = list(payments.values())
        response = {
            'payments': payment_data,
            'totals': {
                'total_rate': total_rate,
                'total_defAmt': total_defAmt,
                'total_tollAmt': total_tollAmt,
                'total_driverAmt': total_driverAmt,
                'total_tyreAmt': total_tyreAmt,
                'total_driverAdvance': total_driverAdvance,
                'grand_total': grand_total1,
                'calculated_value': calculated_value,  # Calculated difference
            }
        }

        # Return structured response with grand total prominently displayed
        return JsonResponse(response, json_dumps_params={'indent': 4})

    except ValueError as e:
        # Handle missing parameter error
        return JsonResponse({'error': str(e)}, json_dumps_params={'indent': 4}, status=400)

    except ObjectDoesNotExist as e:
        # Handle object not found error
        return JsonResponse({'error': 'Object does not exist: ' + str(e)}, json_dumps_params={'indent': 4}, status=404)

    except Exception as e:
        # Catch-all for unexpected errors
        return JsonResponse({'error': 'An unexpected error occurred: ' + str(e)}, json_dumps_params={'indent': 4}, status=500)

def get_vehiclepayment_summary(request, load_id):
    try:
        # Fetch all Vehiclepayment records for the given loadId
        vehicle_payments = Vehiclepayment.objects.filter(loadId=load_id)

        # Calculate the total of specific fields
        total_values = vehicle_payments.aggregate(
            total_defAmt=Sum('defAmt'),
            total_tollAmt=Sum('tollAmt'),
            total_driverAmt=Sum('driverAmt'),
            total_tyreAmt=Sum('tyreAmt'),
            total_driverAdvance=Sum('driverAdvance'),
        )

        # Get the first record for display
        first_record = vehicle_payments.first()
        ...
        response_data = {
            ...
            'first_record': {
                ...
                'vehicleNo': first_record.vehicleNo,
                'route_from': first_record.route_from,
                'route_to': first_record.route_to,
            }
        }
...
def vehiclepayment_summary(request):
    ...
    # Base queryset
    queryset = Vehiclepayment.objects.all()
    ...
    if to_date:
        queryset = queryset.filter(date__lte=to_date)
    if vehicle_no:
        queryset = queryset.filter(vehicleNo__icontains=vehicle_no)

    # Aggregate totals and first row data for each loadId
    summary_data = (
        queryset
        .values('loadId')
        .annotate(
    ...
    # Render the template with the filtered data
    return render(request, 'vehiclepayment_summary.html', {'summary_data': summary_data})


def fetch_vehiclepayment_details(request, load_id):
    # Base queryset for the given loadId
    details = Vehiclepayment.objects.filter(loadId=load_id)
    ...
    if from_date:
        details = details.filter(date__gte=from_date)
    if to_date:
        details = details.filter(date__lte=to_date)
    if vehicle_no:
        details = details.filter(vehicleNo__icontains=vehicle_no)

    return render(request, 'vehiclepayment_details.html', {'details': details})

from django.shortcuts import render, get_object_or_404, redirect
from .models import Vehiclepayment

def edit_vehiclepayment(request, id):
    # Get the record to be edited
    record = get_object_or_404(Vehiclepayment, id=id)

    if record.amtDate:
        record.amtDate = record.amtDate.strftime('%Y-%m-%d')

    if request.method == 'POST':
        # Update the fields with POST data
        record.vehicleNo = request.POST.get('vehicleNo', record.vehicleNo)
        record.advance = request.POST.get('advance', record.advance)
        record.total = request.POST.get('total', record.total)
        record.balance = request.POST.get('balance', record.balance)
        record.paidAmt = request.POST.get('paidAmt', record.paidAmt)
        record.route_from = request.POST.get('route_from', record.route_from)
        record.route_to = request.POST.get('route_to', record.route_to)
        record.amtDate = request.POST.get('amtDate', record.amtDate)
        record.ltr = request.POST.get('ltr', record.ltr)
        record.ltrRate = request.POST.get('ltrRate', record.ltrRate)
        record.rate = request.POST.get('rate', record.rate)
        record.defAmt = request.POST.get('defAmt', record.defAmt)
        record.tollAmt = request.POST.get('tollAmt', record.tollAmt)
        record.driverAmt = request.POST.get('driverAmt', record.driverAmt)
        record.tyreAmt = request.POST.get('tyreAmt', record.tyreAmt)
        record.driverAdvance = request.POST.get('driverAdvance', record.driverAdvance)

        # Save the updated record
        record.save()

        # Redirect to the details page
        return redirect('vehiclepayment_details', load_id=record.loadId)

    # Render the edit template with the existing record data
    return render(request, 'edit_vehiclepayment.html', {'record': record})


def daily_consignment(request):
    if request.method == "POST":
        now = datetime.now().replace(microsecond=0)
        con_date = now.strftime("%Y-%m-%d")
        current_time = now.strftime("%H:%M:%S")

        # Assuming session or direct form input is used to get company name and headname.
        uname = request.session.get('companyname', 'Default Company')  # Default as fallback
        username = request.session.get('headname', 'Default Head')  # Default as fallback
        ...
        return redirect('adminprintConsignment', track_id=con_id)
    return render(request, 'daily_consignment.html')


from django.core.exceptions import ObjectDoesNotExist
from django.utils.safestring import mark_safe

def adminprintConsignment(request, track_id):
    grouped_userdata = {}
    copy_types = []
    totalqty = 0  # Initialize total quantity
    ...
    return render(request, 'adminprintConsignment.html', {
        'grouped_userdata': grouped_userdata,
        'copy_types': ', '.join(copy_types),  # Include the aggregated copy types
        'totalqty': totalqty  # Pass total quantity to the template
    })
...
def adminviewconsignment(request):
    uid = request.session.get('username')
    grouped_userdata = {}
    ...
    if uid:
        try:
            ...
            # Fetch consignments without filtering by branch
            consignments = AddConsignment.objects.all()
    ...
    return render(request, 'adminviewconsignment.html', {'grouped_userdata': grouped_userdata})
...
def adminconsignment_edit(request, pk):
    # Fetch the main consignment object
    userdata = get_object_or_404(AddConsignment, track_id=pk)
    ...
    if request.method == "POST":
        ...
        if userdatatemp:
            userdatatemp.route_from = request.POST.get('from')
            userdatatemp.route_to = request.POST.get('to')
            userdatatemp.sender_name = request.POST.get('a1')
            userdatatemp.receiver_name = request.POST.get('a5')
            userdatatemp.total_cost = request.POST.get('cost')
            userdatatemp.weight = request.POST.get('weight')
        ...
        for product_id, product_desc, piece in zip(product_ids, products_list, pieces_list):
            if product_desc and piece:  # If we have valid product data
                if product_id:  # Update existing product
                    product_queryset = AddConsignment.objects.filter(id=product_id)
        ...
        if userdatatemp:
            for product_idt, product_desct, piecet in zip(product_ids, products_list, pieces_list):
                if product_desct and piecet:  # Ensure valid data
                    product_queryset_temp = AddConsignmentTemp.objects.filter(id=product_idt)
                    if product_queryset_temp.exists():
                        product_obj = product_queryset_temp.first()
                        product_obj.desc_product = product_desct
                        product_obj.pieces = piecet
                        product_obj.save()
                    else:
                        new_product_temp = AddConsignmentTemp(
                            track_id=userdata.track_id,
                            desc_product=product_desct,
                            pieces=piecet,
                            sender_name=userdata.sender_name,
                        ...
                        new_product_temp.save()

        return redirect(reverse('adminviewconsignment'))

    return render(request, 'adminconsignment_edit.html', {'userdata': userdata, 'products': products})
...
def booking(request):
    if request.method == "POST":
        ...
        rate_fixed = float(request.POST.get('rate_fixed'))
        advance = float(request.POST.get('advance'))
        balance = float(request.POST.get('balance'))
        address = request.POST.get('address')
        material = request.POST.get('material')
        aprox_weight = request.POST.get('aprox_weight')
        driver= request.POST.get('driver')
        ...
        # Generate the next loading slip number
        last_booking = Booking.objects.order_by('-id').first()
        if last_booking and last_booking.loading_slip_number:
            next_number = int(last_booking.loading_slip_number) + 1
        else:
            next_number = 1
        loading_slip_number = f"{next_number:03}"  # Format as 001, 002, etc.
        ...
        booking = Booking(
            ...
            paidAmt=advance,
            balance=balance,
            loading_slip_number=loading_slip_number,
            address=address,
            material=material,
        ...
        # Automatically update the status based on balance
...
def edit_booking(request, id):
    data = Booking.objects.filter(id=id).first()  # Retrieve a single object or None
    ...
    if request.method == "POST":
        ...
        company_name = request.POST.get('company_name')
        truck_number = request.POST.get('truck_number')
        rate_fixed = request.POST.get('rate_fixed')
        balance = request.POST.get('balance')
        advance = request.POST.get('advance')
        paidAmt = request.POST.get('paidAmt')
        loading_slip_number = request.POST.get('loading_slip_number')
        remark = request.POST.get('remark')
        address = request.POST.get('address')
        material = request.POST.get('material')
        aprox_weight = request.POST.get('aprox_weight')
        ...
        data.rate_fixed = rate_fixed
        data.balance = balance
        data.advance = advance
        data.paidAmt = paidAmt
        data.loading_slip_number = loading_slip_number
        data.status = status
        data.remark = remark
        data.address = address
        data.aprox_weight = aprox_weight
        data.driver = driver
        data.driver_number = driver_number
        data.owner_name = owner_name
        data.owner_number = owner_number
    ...
    return render(request, 'bookingEdit.html', {'data': data})
...
def booking_report(request, booking_id):
    try:
        ...
        # Pass the booking details to the template for printing
        return render(request, 'booking_report.html', {'booking': booking})
...
def fullLoadprint(request, pk):
    try:
        data = Fullload.objects.get(loadId=pk)  # Use get() for a single object
    except Fullload.DoesNotExist:
        data = None

    return render(request, 'fullLoadprint.html', {'data': data})

from django.shortcuts import render
from django.utils.dateparse import parse_date
from django.core.exceptions import ObjectDoesNotExist
from .models import Booking  # Replace with the actual model import
...
from django.core.exceptions import ObjectDoesNotExist
from django.utils.dateparse import parse_date
from .models import Booking  # Update with your actual model import
...
def booking_balance(request):
    grouped_userdata = {}

    try:
        # Handle GET request by displaying the data or rendering a template
        if request.method == "GET":
            ...
            for booking in bookings:
                slip_number = booking.loading_slip_number
                if slip_number not in grouped_userdata:
                    grouped_userdata[slip_number] = {
                        'from_location': booking.from_location,
                        'to_location': booking.to_location,
                        'date': booking.date,
                        'rate_fixed': booking.rate_fixed,
                        'advance': booking.advance,
                        'paid_amount': booking.paidAmt,
                        'balance': booking.balance,
                        'loading_slip_number': booking.loading_slip_number,
                        'status': booking.status,
                        'booking_id': booking.id,  # Include the booking ID for modal
                    }

            # Return a response, either JSON or a template
            return render(request, 'booking_balance.html', {'grouped_userdata': grouped_userdata})
    ...
    if request.method == "POST" and 'booking_id' in request.POST:
        try:
            ...
            # Update advance and balance
            booking.advance += amount
            new_balance = booking.rate_fixed - booking.advance
            booking.balance = new_balance

            # Update booking status
            booking.status = 'Paid' if new_balance <= 0 else 'Pending'
            ...
            return JsonResponse({
                ...
                'new_advance': str(booking.advance),
            })
...
def update_booking_status(request):
    if request.method == "POST":
        booking_id = request.POST.get('booking_id')  # From the hidden input in the modal
        amount = request.POST.get('amount')  # Amount input in the modal
        remark = request.POST.get('remark')  # Remark input in the modal
        ...
        try:
            ...
            # Get the Fullload entry
            full_load = get_object_or_404(Booking, id=booking_id)
...
def quotation_entry(request):
    if request.method == "POST":
        ...
        truck_details = request.POST.get("truck_details")
        weight_guarantee = request.POST.get("weight_guarantee")
        freight_rate = request.POST.get("freight_rate")
        freight_payable_by = request.POST.get("freight_payable_by")

        # Get the last saved quotation and determine the new quot_id
        last_quotation = Quotation.objects.order_by('-id').first()
        new_quot_id = last_quotation.quot_id + 1 if last_quotation else 1  # Start from 1 if no previous record

        # Save to database with auto-incremented quot_id
        Quotation.objects.create(
            ...
            weight_guarantee=weight_guarantee,
            freight_rate=freight_rate,
            freight_payable_by=freight_payable_by,
        )

        return redirect(reverse('quotation_entry'))

    return render(request, "quotation_entry.html")
...
def quotation_list(request):
    from_date = request.GET.get('from_date')
    to_date = request.GET.get('to_date')
    bookings = Quotation.objects.all()
    ...
    return render(request, 'quotation_list.html', {'bookings': bookings})


def quotation(request, quot_id):
    try:
        booking = Quotation.objects.get(id=quot_id)

        # Pass the booking details to the template for printing
        return render(request, 'quotation.html', {'booking': booking})
...
def edit_quotation(request, id):
    data = Quotation.objects.filter(id=id).first()  # Retrieve a single object or None
    ...
    if request.method == "POST":
        ...
        data.truck_details = truck_details
        data.weight_guarantee = weight_guarantee
        data.address = address
        data.freight_rate = freight_rate
        data.freight_payable_by = freight_payable_by
